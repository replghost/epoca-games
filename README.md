...
`libresector/LICENSE`. Commercial Duke Nukem 3D game data is not distributed
and may only be supplied by the user as a local Content override. The original
Duke Nukem 3D v1.3d shareware ZIP may be staged only as the exact, unmodified
distribution; its included `LICENSE.TXT` governs redistribution.
